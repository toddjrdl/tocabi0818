...
#################################################################################
    def train_actor_critic(self, obs_dict, opt_step=True):
        ...
        if recon is not None:
            # MSE reconstruction loss: ||s̃_t - s_t||²
            reconstruction_loss = F.mse_loss(recon, obs_dict['states'][:, -1, :])
            
            # L1 regularization on latent embeddings: λ_r||z_t||₁
            latent_z = res.get('z', None)  # 모델에서 latent embedding을 리턴해야 함
            if latent_z is not None:
                lambda_r = self.config.get('lambda_r', 0.002)  # 논문 Table VIII 값
                l1_reg = torch.mean(torch.abs(latent_z))
                denoise_loss = reconstruction_loss + lambda_r * l1_reg
            else:
                denoise_loss = reconstruction_loss
                print("Warning: latent_z not found in model output, using reconstruction loss only")
        else:
